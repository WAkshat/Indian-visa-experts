...
- **Contact form** (`components/forms/ContactForm.tsx`) — on submit, builds a fully-formatted email (name, email, nationality, phone, service, message) and opens the visitor's mail app pre-addressed to `indiavisaxperts@gmail.com`. Success state updated to reflect this and to show the direct email as a fallback.
